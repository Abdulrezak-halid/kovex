...
| Yayına Alma Katmanı | Render Web Service, Supabase PostgreSQL |
...
| Yayınlama / Hosting | Render Web Service |
...
Kovex ERP, sistemin yalnızca yerel geliştirme ortamında değil, gerçek bir çevrimiçi ortamda da çalışabildiğini göstermek amacıyla canlı bir web uygulaması olarak yayına alınmıştır. Seçilen yayına alma stratejisinde web servisi için Render, bulut PostgreSQL veritabanı için Supabase kullanılmıştır.
...
Yayına alma mimarisi aynı origin yaklaşımını kullanmaktadır. React frontend uygulaması statik dosyalar olarak build edilmekte ve Express backend tarafından servis edilmektedir. Backend, REST API endpoint'lerini `/api` altında, Swagger UI dokümantasyonunu `/api-docs` altında sunmakta ve Supabase PostgreSQL veritabanına güvenli üretim bağlantı bilgisi ile bağlanmaktadır.

| Yayına Alma Alanı | Teknoloji / Servis |
| --- | --- |
| Web Uygulaması Hosting | Render Web Service |
| Backend Runtime | Docker tabanlı dağıtım içinde Node.js ve Express |
| Frontend Sunumu | Build edilmiş Vite frontend'in Express uygulaması tarafından servis edilmesi |
| Bulut Veritabanı | Supabase PostgreSQL |
| Veritabanı Erişimi | PostgreSQL bağlantısı ile Drizzle ORM |
| API Dokümantasyonu | Yayındaki backend üzerinden erişilebilen Swagger UI |
| Ortam Değişkenleri Yönetimi | Üretim gizli bilgileri için Render environment variables |

Yayına alma sürecinde veritabanı bağlantı adresi ve kimlik doğrulama gizli anahtarı gibi hassas değerler environment variable olarak saklanmıştır. Bu değerler kaynak kod deposuna eklenmemiştir. Bu yaklaşım güvenliği artırmakta ve yayına alma konusunda yaygın iyi uygulamalarla uyumludur.

Bu canlı dağıtım, bitirme projesi açısından önemlidir çünkü Kovex ERP'nin yalnızca yerel bir prototip olmadığını göstermektedir. Sistem build edilebilmekte, yayına alınabilmekte, bulut veritabanına bağlanabilmekte ve değerlendirme/demonstrasyon amacıyla herkese açık bir URL üzerinden erişilebilmektedir.
