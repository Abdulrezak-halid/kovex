...
export default function Orders() {
  ...
  const columns = [
    { header: "Reference", cell: (r: Order) => <span className="font-medium font-mono text-sm">{r.reference}</span> },
    { header: "Customer", cell: (r: Order) => r.customerName },
    { header: "Status", cell: (r: Order) => <StatusBadge status={r.status} /> },
    { header: "Total", cell: (r: Order) => <span className="font-medium">{fmt(r.totalAmount)}</span> },
    { header: "Date", cell: (r: Order) => new Date(r.createdAt).toLocaleDateString() },
    {
      header: "Actions",
      cell: (r: Order) => (
        <div className="flex gap-1">
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7 text-primary" onClick={() => handleCreateInvoice(r.id)} title="Create Invoice">
  ...
  return (
    <div className="p-6">
      ...
      />
      <DataTable columns={columns} data={data} isLoading={isLoading} keyField="id" emptyMessage="No orders." />
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={(!editing && (!customerId || items.length === 0)) || createMutation.isPending || updateMutation.isPending}>
