...
export default function Suppliers() {
  ...
  const columns = [
    { header: "Name", cell: (r: Supplier) => <span className="font-medium">{r.name}</span> },
    { header: "Company", cell: (r: Supplier) => r.company || <span className="text-muted-foreground">—</span> },
    { header: "Email", cell: (r: Supplier) => r.email || <span className="text-muted-foreground">—</span> },
    { header: "Phone", cell: (r: Supplier) => r.phone || <span className="text-muted-foreground">—</span> },
    {
      header: "Actions",
      cell: (r: Supplier) => (
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-4 relative w-64">
        ...
        <Input placeholder="Search suppliers..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-sm" />
      </div>
      <DataTable columns={columns} data={data} isLoading={isLoading} keyField="id" emptyMessage="No suppliers." />
        ...
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            {(["name", "company", "email", "phone", "address"] as const).map((field) => (
              <div key={field}><Label className="capitalize">{field}</Label><Input className="mt-1" value={(form as any)[field]} onChange={(e) => setForm((f) => ({ ...f, [field]: e.target.value }))} /></div>
            ))}
          </div>
          <DialogFooter>
