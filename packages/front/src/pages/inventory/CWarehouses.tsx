...
export default function CWarehouses() {
    ...
    try {
      if (editing) {
        await updateMutation.mutateAsync({
          id: editing.id,
          data: { name, location: location || undefined } as any,
        });
      } else {
  ...
  const columns = [
    {
      header: "Name",
      cell: (r: Warehouse) => <span className="font-medium">{r.name}</span>,
    },
    {
      header: "Location",
      cell: (r: Warehouse) =>
    ...
    {
      header: "Created",
      cell: (r: Warehouse) => new Date(r.createdAt).toLocaleDateString(),
    },
    {
      header: "Actions",
      cell: (r: Warehouse) => (
