...
export default function Quotations() {
  ...
  const [items, setItems] = useState<{ productId: number; productName: string; quantity: number; unitPrice: number }[]>([]);
  ...
  function openEdit(row: Quotation) {
    ...
    setNotes(row.notes ?? "");
    setItems(row.items.map((i) => ({ productId: i.productId, productName: i.productName, quantity: i.quantity, unitPrice: i.unitPrice })));
    setDialogOpen(true);
  }
  ...
  function updateItem(idx: number, field: string, value: string | number) {
    setItems((prev) => {
      ...
      if (field === "productId") {
        ...
        next[idx] = { ...next[idx], productId: Number(value), productName: p?.name ?? "", unitPrice: Number(p?.price ?? 0) };
      } else {
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        await updateMutation.mutateAsync({ id: editing.id, data: { status, notes, items: payload.items } as any });
      } else {
  ...
  const columns = [
    { header: "Reference", cell: (r: Quotation) => <span className="font-medium font-mono text-sm">{r.reference}</span> },
    { header: "Customer", cell: (r: Quotation) => r.customerName },
    { header: "Status", cell: (r: Quotation) => <StatusBadge status={r.status} /> },
    { header: "Total", cell: (r: Quotation) => <span className="font-medium">{fmt(r.totalAmount)}</span> },
    { header: "Date", cell: (r: Quotation) => new Date(r.createdAt).toLocaleDateString() },
    {
      header: "Actions",
      cell: (r: Quotation) => (
        <div className="flex gap-1">
          ...
          {(r.status === "accepted" || r.status === "draft") && (
            <Button variant="ghost" size="icon" className="h-7 w-7 text-primary" onClick={() => handleConvert(r.id)} title="Convert to Order">
  ...
  return (
    <div className="p-6">
      ...
      />
      <DataTable columns={columns} data={data} isLoading={isLoading} keyField="id" emptyMessage="No quotations." />
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl">
              ...
              <div className="space-y-2">
                {items.map((item, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                    ...
                    <div className="col-span-3">
                      <Input type="number" placeholder="Price" min={0} step={0.01} value={item.unitPrice} onChange={(e) => updateItem(idx, "unitPrice", Number(e.target.value))} />
                    </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={!customerId || items.length === 0 || createMutation.isPending || updateMutation.isPending}>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Delete quotation?</AlertDialogTitle><AlertDialogDescription>This cannot be undone.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter>
