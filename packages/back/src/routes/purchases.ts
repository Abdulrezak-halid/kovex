  ...
  try {
    ...
    let warehouses = req.body?.warehouseId
      ? await db
          ...
          .where(eq(warehousesTable.id, Number(req.body.warehouseId)))
          .limit(1)
      ...
      const existing = await db
        ...
        .where(
          and(
            ...
            eq(stockTable.warehouseId, whId),
          ),
        )
        .limit(1);
  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        reference: purchaseInvoicesTable.reference,
        supplierId: purchaseInvoicesTable.supplierId,
        supplierName: suppliersTable.name,
        purchaseOrderId: purchaseInvoicesTable.purchaseOrderId,
      ...
      .innerJoin(
        suppliersTable,
        eq(purchaseInvoicesTable.supplierId, suppliersTable.id),
      )
...
router.post("/purchase-invoices", async (req, res) => {
  try {
    const { supplierId, purchaseOrderId, totalAmount, dueDate, notes } =
      req.body;
    const [inv] = await db
      .insert(purchaseInvoicesTable)
      .values({
        reference: genRef("PINV"),
        supplierId: Number(supplierId),
